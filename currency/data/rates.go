...
func (e *ExchangeRates) GetRate(base string, dest string) (float64, error) {
	br, ok := e.rates[base]
	if !ok {
		return 0, fmt.Errorf("Rate not found for current %s", base)
	}

	dr, ok := e.rates[dest]
	if !ok {
		return 0, fmt.Errorf("Rate not found for current %s", dest)
	}
...
// in rates for demostration purposes
		...
		for {
			...
			case <-ticker.C:
				...
				for k, v := range e.rates {
					// changes can be 10% of original value
					change := (rand.Float64() / 10)
					// is this a positive or negative change
					direction := rand.Intn(1)

					if direction == 0 {
						// new value with the min 90% of old
						change = 1 - change
					} else {
						// new value with the min 90% of old
						change = 1 + change
...
func (e *ExchangeRates) getRates() error {
	resp, err := http.DefaultClient.Get(
		"https://www.ecb.europa.eu/stats/eurofxref/eurofxref-daily.xml?9e8dc18235aa9c94e398417d97d6360f",
	)
	if err != nil {
		return nil
	}
